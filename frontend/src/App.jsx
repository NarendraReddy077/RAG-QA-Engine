...
function App() {
  ...
  return (
    <div className="flex h-screen App font-sans">
      {/* Sidebar */}
      <div className="w-72 sidebar flex flex-col hidden md:flex transition-all duration-300">
        <div className="p-4 sidebar-header flex items-center justify-between">
          <h2 className="text-xl font-bold history-title">
            History
          </h2>
          <button
            onClick={() => setCurrentSessionId(null)}
            className="p-2 new-chat-btn rounded-lg transition-colors"
            title="New Chat"
          >
            <Plus size={20} />
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-1">
          ...
          {sessions.map((s) => (
            <div
              ...
              className={`w-full text-left px-3 py-2.5 rounded-lg transition-colors flex items-center justify-between group cursor-pointer session-item ${currentSessionId === s.id ? 'active' : ''}`}
            >
              <div className="truncate flex-1 text-sm font-medium pr-2">{s.title}</div>
              <button
                onClick={(e) => handleDeleteSession(s.id, e)}
                className="opacity-0 group-hover:opacity-100 p-1 delete-btn rounded transition-all flex-shrink-0"
                title="Delete Chat"
              >
                <Trash2 size={14} />
              </button>
        ...
        <div className="p-4 border-t border-gray-200 dark:border-zinc-800 flex flex-col gap-2">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 add-knowledge-btn py-2.5 rounded-xl transition-all shadow-md hover:shadow-lg font-medium"
          >
            <Database size={18} />
            Add Knowledge
          </button>
          <button
            ...
            className="w-full flex items-center justify-center gap-2 manage-knowledge-btn py-2.5 rounded-xl transition-all shadow-sm font-medium"
          >
            <List size={18} />
            Manage Knowledge
          </button>
      ...
      {/* Main Chat Area */}
      <div className="flex-1 flex flex-col relative chat-area">
        {/* Header */}
        <header className="h-16 chat-header flex items-center px-6 justify-between backdrop-blur-md sticky top-0 z-10">
          <h1 className="text-lg text-green-500 font-semibold flex items-center gap-2">
            RAG Engine
            {currentSessionId && <span className="text-sm px-2 py-0.5 rounded-full bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300">Active</span>}
          </h1>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6">
          {messages.length === 0 && !isLoading && (
            <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto fade-in">
              <div className="w-16 h-16 bg-green-100 dark:bg-green-900/40 rounded-full flex items-center justify-center mb-4 text-green-600 dark:text-green-400 shadow-inner">
                <Database size={32} />
              </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-gray-100 dark:border-zinc-800">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {msg.sources.map((src, i) => (
                        <div key={i} className="text-xs px-2 py-1 sources rounded text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-zinc-700 flex items-center gap-1 cursor-help group relative">
          ...
          {isLoading && (
            <div className="flex justify-start mb-6">
              <div className="message-assistant rounded-tl-none p-4 shadow-sm flex items-center gap-3">
                <Loader2 className="animate-spin loading-spinner" size={20} />
        ...
        {/* Input Area */}
        <div className="p-4 md:p-2 input-area">
          <div className="max-w-4xl mx-auto">
            <form
              className="relative flex items-center border border-gray-300 dark:border-zinc-700 rounded-2xl bg-white dark:bg-zinc-900 shadow-sm focus-within:ring-2 focus-within:ring-green-500 focus-within:border-green-500 transition-all"
              onSubmit={handleSend}
            >
              <input
                type="text"
                ...
                className="flex-1 input-field px-6 py-4 outline-none placeholder-gray-400 text-sm md:text-base"
              ...
              <button
                ...
                className="mx-3 send-btn disabled:bg-gray-300 disabled:dark:bg-zinc-700 p-2.5 rounded-xl transition-colors shadow-sm"
              >
                <Send size={18} />
              </button>
            </form>
      ...
      {/* Ingest Modal */}
      {ingestModalOpen && (
        <div className="fixed inset-0 modal-overlay backdrop-blur-sm z-50 flex items-center justify-center fade-in">
          <div className="modal-content w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleIngest} className="p-6">
              <div className="mb-4">
                ...
                <select
                  ...
                  className="w-full bg-white dark:bg-zinc-950 border border-gray-300 dark:border-zinc-700 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-500 outline-none transition-shadow"
                >
              ...
              <div className="mb-6">
                ...
                {ingestType === 'url' ? (
                  ...
                  <input
                    ...
                    className="w-full bg-white dark:bg-zinc-950 border border-gray-300 dark:border-zinc-700 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-500 outline-none transition-shadow file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-green-50 file:text-green-700 hover:file:bg-green-100"
                    required
                  />
                ) : (
                  <textarea
                    value={ingestContent}
                    onChange={(e) => setIngestContent(e.target.value)}
                    className="w-full bg-white dark:bg-zinc-950 border border-gray-300 dark:border-zinc-700 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-500 outline-none transition-shadow h-32 resize-none"
              ...
              <div className="flex gap-3 justify-end">
                ...
                <button
                  ...
                  className="px-4 py-2 text-sm font-medium bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:bg-green-400 transition-colors flex items-center gap-2 shadow-sm"
                >
      ...
      {manageKnowledgeOpen && (
        <div className="fixed inset-0 modal-overlay backdrop-blur-sm z-50 flex items-center justify-center fade-in">
          <div className="modal-content w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            ...
            <div className="p-6 overflow-y-auto flex-1">
              {isLoadingKnowledge ? (
                <div className="flex justify-center items-center py-8">
                  <Loader2 className="animate-spin loading-spinner" size={24} />
                </div>
